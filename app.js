...
window.renderQuizzes = function() {
        ...
        if (!mistakesReport || mistakesReport.length === 0) {
            contentArea.innerHTML = `
                <div class="question-card" style="text-align: center; border-top: 4px solid #22c55e; padding: 40px 20px;">
                    ...
                    <p style="color: #94a3b8;">Học sinh chưa làm sai ý trắc nghiệm nào hoặc đã xuất sắc khắc phục hết các lỗi trước đó!</p>
                </div>
            `;
            return;
        }
            ...
            <div class="question-card" id="card_${quiz.id}" style="border-top-color: ${borderTopColor}">
                ...
                <div class="statements-container">
                    ${quiz.statements.map((s, sIndex) => {
                        // ĐÃ SỬA: Tự động nhận diện hoặc sinh nhãn lỗi a), b), c), d) chuẩn xác
                        ...
                        <div class="statement" id="stmt_${quiz.id}_${s.id}">
                            <div class="stmt-row">
                                <div class="statement-text">${labelText}${cleanText} ${alertBadge}</div>
                                <div class="options">
                                    <label class="opt-btn">
                ...
                <button class="btn-check" style="${isMistakeTab ? 'background-color: #991b1b; border-color: #ef4444;' : ''}" onclick="checkAnswers('${quiz.id}')">Kiểm tra kết quả</button>
            </div>
        `;
    }).join('');

    let submitHtml = '';
    if (!isMistakeTab && currentQuizzes.length > 0) {
        submitHtml = `
    ...
    // Đổ toàn bộ Lý thuyết + Tiêu đề + Câu hỏi + Nút nộp bài ra màn hình
            ...
            <div class="score-modal-box">
                ...
                <div class="modal-body">
                    ...
                    <div class="score-table-wrapper">
                        <h3 style="color: #f1f5f9; margin-bottom: 12px; font-size: 1.1rem; border-bottom: 1px solid #334155; padding-bottom: 8px;">📋 Bảng thống kê chi tiết</h3>
